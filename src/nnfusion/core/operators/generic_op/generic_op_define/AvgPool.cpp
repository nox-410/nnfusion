    ...
    */
    .translate_v2([](std::shared_ptr<graph::GNode> curr) -> std::string {
        ...
        auto expression_template =
            R"( @output0@@output0_layout@ +=! @input0@@input0_layout@@when_condition@ / @window_size@@where_condition@; )";
        ...
        size_t window_size = nnfusion::shape_size(strides);
        if (is_nchw)
        {
        ...
        config["when_condition"] = when_condition;
        config["where_condition"] = where_condition;
        ...
        config["window_size"] = to_string(window_size);
